...
desc: Going in-depth how 2D differentiable rendering/rasterization works and reimplementing it using PyTorch.
...
Now to me personally it isn't 100% clear why `atan` and `atanx` fails this badly.
But I think it's somewhat safe to assume that that's caused because `atan` decays slowly, which causes the optimizer to try and optimize for "everything" at once, thus resulting in a very mediocre average.
...
We also have seen that `sigmoid` does overall end up with a better generalization, but `smoothstep` gives us crispy edges. So what if we mix them both?  
We could also try to optimize in multiple steps, instead of doing it in one go we do smaller optimization runs each one trying to decrease the difference of the target image.
...
Well this concludes this blog post, I believe that differentiable rendering perfectly illustrates how there really isn't a true right or wrong in ML, and that things that work great in one scenario work completely different in another.  
