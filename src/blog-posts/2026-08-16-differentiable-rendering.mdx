...
Now to me personally it isn't 100% clear why `atan` and `atanx` fails this badly, but I think it's somewhat safe to assume that that's caused because of `atan` decaying slowly thus having large gradients.
Which causes the optimizer to try and optimize for "everything" at once, thus resulting in a very mediocre average. 
