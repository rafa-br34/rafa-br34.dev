...
desc: Going in depth on how 2D differentiable rendering/rasterization works and reimplementing it using PyTorch.
date: 2026-08-16T16:40-03:00
edit: 2026-09-05T23:26-03:00
tags: ["ai/ml", "rasterization", "gradients"]
thumbnail: out-optim_target_o_pastelzera_headshot.png-norm_sigmoid-1788407794-animated-no-bar.webp
...
Now, notice how, when the function isn't normalized, it defines a gradient in which the sign indicates whether we are inside or outside the shape.
You'll also notice the undulating stripes in the visualization; those are the sine of the function output ($sin(z)$).
They don't have a specific use in our case, but they do visually encode the gradient field, making it easier to see how the SDF changes smoothly across space.  
...
Much like in GPUs, where each pixel is evaluated in parallel, the exact same can be done in PyTorch.  
So we can begin by defining a grid with size (width, height) in the range (0 to (width - 1), 0 to (height - 1)), like so:  
...
### Applying an SDF function to it

Now we can simply apply an SDF function to each one of those values (much like how you would do in a GPU shader).  
...
So we have to define a target to optimize for; let's begin by just using a circle (which can be defined by our own circle SDF).
...
1. The gradient is basically back-propagation telling us how we should change our parameters (so it's a delta). Thus, we start by defining a tensor that stores that delta (same size as the number of parameters), for every sample we want to get and for every type of function we have (which is 6: `raw`, `atan`, `atanx4`, `tanh`, `sigmoid`, `smoothstep`). We do the same with loss, but loss is represented by a single number.
2. We iterate over every point we would like to sample and track its index (we have to know which sample the current gradient vector and loss belong to).
...
Here we are using $p_x = 48, p_y = 48$
![Adam trying to optimize when the circle is closer to the corner](test-run-offcenter-adam.webp)
Well, that's interesting, `atan` and `atanx4` converge because their gradients propagate a lot further than the other functions.  

### Multiple objects

So now that we have the foundations down, let's use multiple objects and a different line SDF.
...
Using the same method for the circle SDF, we can make it into an optimization target:
...
Now with the optimization fundamentals out of the way, let's add color.  
Adding color is quite straightforward since our `sdf_lines` function returns a tensor of size `[W, H, M]` where `M` is the number of lines we have.  
So we can use a static canvas and basically write each shape into it by first subtracting our shape from the canvas (so we get a black void where our shape would go) and then adding it to the canvas (thus filling the void with our shape).  
...
And again, `sigmoid` is the one that gets closest to convergence first. Surprisingly, most of the functions get close to convergence, but sigmoid gets there pretty fast.  
...
<img alt="Image of my OC as the first optimization target" src="optim_target_o_pastelzera_headshot.png" height="200" style={{height: 200}}/>
<img alt="Image of my OC as the second optimization target" src="optim_target_gummi_art.png" height="200" style={{height: 200}}/>

Based on what we understand now, I put together the following script:

```py filename="optimizer_simple.py"
import random
import subprocess
import sys
import time
...
So to begin with, I tested multiple normalizations.  
...
From these results, we can clearly see that `smoothstep`, `tanh`, and `sigmoid` work the best. While `atan` and `atanx` fail very poorly at it.  
Now, to me personally, it isn't 100% clear why `atan` and `atanx` fail this badly.
But I think it's somewhat safe to assume that that's because `atan` decays slowly, which causes the optimizer to try and optimize for "everything" at once, thus resulting in a very mediocre average.

### Final results

`smoothstep`:  
![optim_target_gummi_art.png norm_smoothstep final result](out-optim_target_gummi_art.png-norm_smoothstep-1788488037-final.png)
![optim_target_gummi_art.png norm_smoothstep animated result](out-optim_target_gummi_art.png-norm_smoothstep-1788488037-animated.webp)

`tanh`:  
![optim_target_gummi_art.png norm_tanh final result](out-optim_target_gummi_art.png-norm_tanh-1788479135-final.png)
![optim_target_gummi_art.png norm_tanh animated result](out-optim_target_gummi_art.png-norm_tanh-1788479135-animated.webp)

`sigmoid`:  
![optim_target_gummi_art.png norm_sigmoid final result](out-optim_target_gummi_art.png-norm_sigmoid-1788483936-final.png)
![optim_target_gummi_art.png norm_sigmoid animated result](out-optim_target_gummi_art.png-norm_sigmoid-1788483936-animated.webp)


## Possible improvements

So... we have proven it works, and the idea *is* solid.  
So what's next?  

1. We barely touched the hyperparameters, optimizers, and how initialization is performed.  
We have also seen that `sigmoid` does overall end up with better generalization, but `smoothstep` gives us crisp edges. So what if we mix them both?  
We could also try to optimize in multiple steps; instead of doing it in one go, we do smaller optimization runs, each one trying to decrease the difference from the target image.
2. It *urgently* needs to be optimized; we are allocating a tensor of size `WxH` for *every* line, no matter if that line is visible or has a radius of zero. Which, as you can imagine, isn't great when we're trying to optimize thousands of lines on larger images (a 550x220 image with 5k lines takes up basically 30GiB of VRAM, which shows its inefficiencies).  
We can optimize it by splitting the area in which lines reside into 4 quadrants in the image, but this also has its own drawbacks (how will we deal with lines that go outside those quadrants?).  
...
I'm pretty sure DiffVG and similar are clever enough to pretty much not have this issue, but I haven't studied them in-depth yet, so I can't say for certain.
3. Doing cooler stuff.  
No, but seriously, so far we just have optimized 2D images into 2D images; this same idea can be used for 3D geometry.
However, that will be for a much later date.

Of course, we could solve most of these problems just by cloning DiffVG, but what's the fun in that?  
I would much rather test out [triton](https://triton-lang.org/main/index.html) or [taichi](https://www.taichi-lang.org/) than just compiling DiffVG.  
That would also mean finding out the derivatives of complex functions by hand, but that's something I will need to learn one time or another either way.

## Conclusion

Well, this concludes this blog post. I believe that differentiable rendering perfectly illustrates how there really isn't a true right or wrong in ML, and that things that work great in one scenario work completely differently in another.  
To give my own personal opinion about this, it isn't just about how useful differentiable rendering truly is. I think this is more about understanding and demonstrating why gradient descent is such a powerful algorithm for optimization problems.
As you can imagine, this isn't limited to just optimizing lines to raster images.

## Acknowledgements

Special thanks to [Exulan](https://exulan.com) for providing the compute resources needed for this!  
Demonstration images used courtesy of [o_pastelzera](https://linktr.ee/o_pastelzera) and [Gummi_art](https://bsky.app/profile/gummiart.bsky.social).
